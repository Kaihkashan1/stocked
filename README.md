...
- **Rate limits and busy responses.** On Vercel, each import makes a single Gemini `generate_content` call (about 50s timeout) rather than aborting early and retrying, so a slow save still counts as one request. A daily-quota 429 surfaces the quota message below; a 503/504 capacity blip surfaces "Gemini is busy right now. Wait a few seconds and try again." If a 429 really is the daily quota and `GEMINI_API_KEY_2` is set, the app falls back to the second key automatically before surfacing anything to the user — see section 2. A request that runs past Vercel's 60s function limit can still fail as a platform timeout or a client 499.
